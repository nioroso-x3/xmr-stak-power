...
static inline void sub_word_and_rotr(__m128i* key){
   ...
   *key = vec_perm(*key,*key,(__m128i){0x0,0x1,0x2,0x3, 0x5,0x6,0x7,0x4, 0x8,0x9,0xa,0xb, 0xd,0xe,0xf,0xc});
}

static inline void sub_word_and_rotl(__m128i* key){
   *key =(__m128i)__builtin_crypto_vsbox(*key);
   *key = vec_perm(*key,*key,(__m128i){0x3,0x0,0x1,0x2, 0x4,0x5,0x6,0x7, 0xb,0x8,0x9,0xa, 0xc,0xd,0xe,0xf});
}
...
__m128i soft_aeskeygenassist(__m128i key, uint8_t rcon)
{
  ...
  __m128i rconv = (__m128i){0,0,0,0, rcon,0,0,0, 0,0,0,0, rcon,0,0,0};
...
__m128i soft_aeskeygenassist_be(__m128i key, uint8_t rcon)
{
  ...
  __m128i rconv = (__m128i){0,0,0,rcon, 0,0,0,0, 0,0,0,rcon, 0,0,0,0};
  return vec_xor(key,rconv);
}
